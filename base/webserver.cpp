...
void webserver::dealevent() {
    ...
    LOG_WRITE("webserver start at port: " + to_string(MY_PORT));
    int n = 0;
    while(server_run) {
        // cout << "!!新的循环:"<< n++ << endl;
        ...
        for(int i = 0; i < number; i++) {
            ...
            if(sockfd == listenfd) {  // 确认socket文件是listenfd的，并处理
                LOG_WRITE("new connection!: " + to_string(sockfd) + " usernum: " + to_string(http::user_num));
                dealnewcon();
            }
            else if (events[i].events & (EPOLLRDHUP | EPOLLHUP | EPOLLERR)) {//客户端断开连接
                removefd(epollfd, sockfd);
                http::user_num--;
                LOG_WRITE("client disconnected! usernum: " + to_string(http::user_num));
            }
